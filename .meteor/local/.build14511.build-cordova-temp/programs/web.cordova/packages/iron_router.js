      ...
      return function () {                                                                            // 225
        ...
        try {                                                                                         // 235
          ...
          self.stop = oldStop;                                                                        // 238
                                                                                                      ...
                                                                                                      // 242
    // outer most computation is just used to stop inner computations from one                        // 243
    // place. i don't expect this computation to be invalidated during the run.                       // 244
    self._computation = Deps.autorun(withNoStopsAllowed(function (c) {                                // 245
      Deps.autorun(withNoStopsAllowed(function (onRunComp) {                                          // 246
        if (!self.router._hasJustReloaded && c.firstRun && onRunComp.firstRun)                        // 247
          self.runHooks('onRun');                                                                     // 248
        self.router._hasJustReloaded = false;                                                         // 249
      }));                                                                                            // 250
                                                                                                      // 251
      Deps.autorun(function (c) {                                                                     // 252
        // waitOn                                                                                     // 253
        var waitOnList = self.lookupWaitOn();                                                         // 254
        var waitOn = _.flatten(_.map(waitOnList, function (fnOrHandle) {                              // 255
          return _.isFunction(fnOrHandle) ? fnOrHandle.call(self) : fnOrHandle;                       // 256
        }));                                                                                          // 257
                                                                                                      // 258
        log('waitOn');                                                                                // 259
                                                                                                      // 260
        self.wait(waitOn);                                                                            // 261
      });                                                                                             // 262
                                                                                                      // 263
      Deps.autorun(function (c) {                                                                     // 264
        // if we're already in a renderig transaction we want to cancel the                           // 265
        // transaction. So the previous afterFlush callback should just be a                          // 266
        // noop, and the new afterflush callback should do what's required. But                       // 267
        // we need to keep a stack of these                                                           // 268
        self.router._layout.beginRendering(function onCompleteRenderingTransaction (usedRegions) {    // 269
          if (self.isStopped)                                                                         // 270
            return;                                                                                   // 271
          var allRegions = layout.regionKeys();                                                       // 272
          var unusedRegions = _.difference(allRegions, usedRegions);                                  // 273
          _.each(unusedRegions, function (r) { layout.clear(r); });                                   // 274
        });                                                                                           // 275
                                                                                                      // 276
        // action                                                                                     // 277
        var action = _.isFunction(self.action) ? self.action : self[self.action];                     // 278
        Utils.assert(action,                                                                          // 279
          "You don't have an action named \"" + self.action + "\" defined on your RouteController");  // 280
                                                                                                      // 281
        // Set layout to configured layoutTemplate                                                    // 282
        self.layout(self.lookupLayoutTemplate(), {                                                    // 283
          data: self.lookupProperty('data')                                                           // 284
        });                                                                                           // 285
                                                                                                      // 286
        self.runHooks('onBeforeAction', [], function (paused) {                                       // 287
          if (self.isStopped)                                                                         // 288
            return;                                                                                   // 289
                                                                                                      // 290
          if (!paused) {                                                                              // 291
            action.call(self);                                                                        // 292
                                                                                                      // 293
            if (!self.isStopped) {                                                                    // 294
              self.runHooks('onAfterAction');                                                         // 295
            }                                                                                         // 296
          }                                                                                           // 297
        });                                                                                           // 298
      });                                                                                             // 299
