...
_.extend(Meteor, {                                                                                          // 16
  ...
  _delete: function (obj /*, arguments */) {                                                                // 46
    ...
    for (var i = stack.length - 1; i >= 0; i--) {                                                           // 61
      ...
      delete stack[i][key];                                                                                 // 70
                                                                                                            ...
                                                                                                            // 73
  // _wrapAsync can wrap any function that takes some number of arguments that                              // 74
  ...
  //                                                                                                        // 81
  _wrapAsync: function (fn) {                                                                               // 82
    return function (/* arguments */) {                                                                     // 83
      var self = this;                                                                                      // 84
      var callback;                                                                                         // 85
      var fut;                                                                                              // 86
      var newArgs = _.toArray(arguments);                                                                   // 87
                                                                                                            // 88
      var logErr = function (err) {                                                                         // 89
        if (err)                                                                                            // 90
          return Meteor._debug("Exception in callback of async function",                                   // 91
                               err.stack ? err.stack : err);                                                // 92
      };                                                                                                    // 93
                                                                                                            // 94
      // Pop off optional args that are undefined                                                           // 95
      while (newArgs.length > 0 &&                                                                          // 96
             typeof(newArgs[newArgs.length - 1]) === "undefined") {                                         // 97
        newArgs.pop();                                                                                      // 98
      }                                                                                                     // 99
      // If we have any left and the last one is a function, then that's our                                // 100
      // callback; otherwise, we don't have one.                                                            // 101
      if (newArgs.length > 0 &&                                                                             // 102
          newArgs[newArgs.length - 1] instanceof Function) {                                                // 103
        callback = newArgs.pop();                                                                           // 104
      } else {                                                                                              // 105
        if (Meteor.isClient) {                                                                              // 106
          callback = logErr;                                                                                // 107
        } else {                                                                                            // 108
          fut = new Future();                                                                               // 109
      ...
      newArgs.push(Meteor.bindEnvironment(callback));                                                       // 113
      var result = fn.apply(self, newArgs);                                                                 // 114
      if (fut)                                                                                              // 115
        return fut.wait();                                                                                  // 116
      return result;                                                                                        // 117
    };                                                                                                      // 118
                                                                                                            ...
                                                                                                            // 120
  // Sets child's prototype to a new object whose prototype is parent's                                     // 121
  // prototype. Used as:                                                                                    // 122
  //   Meteor._inherits(ClassB, ClassA).                                                                    // 123
  //   _.extend(ClassB.prototype, { ... })                                                                  // 124
  // Inspired by CoffeeScript's `extend` and Google Closure's `goog.inherits`.                              // 125
  _inherits: function (Child, Parent) {                                                                     // 126
    // copy Parent static properties                                                                        // 127
    for (var key in Parent) {                                                                               // 128
      // make sure we only copy hasOwnProperty properties vs. prototype                                     // 129
      // properties                                                                                         // 130
      if (_.has(Parent, key))                                                                               // 131
        Child[key] = Parent[key];                                                                           // 132
    }                                                                                                       // 133
                                                                                                            // 134
    // a middle member of prototype chain: takes the prototype from the Parent                              // 135
    var Middle = function () {                                                                              // 136
      this.constructor = Child;                                                                             // 137
    };                                                                                                      // 138
    Middle.prototype = Parent.prototype;                                                                    // 139
    Child.prototype = new Middle();                                                                         // 140
    Child.__super__ = Parent.prototype;                                                                     // 141
    return Child;                                                                                           // 142
  }                                                                                                         // 143
});                                                                                                         // 144
                                                                                                            // 145
 ...
 * @locus Anywhere                                                                                          // 47
 * @class                                                                                                   // 48
 * @param {Number} error A numeric error code, likely similar to an HTTP code (eg, 404, 500).               // 49
 * @param {String} [reason] Optional.  A short human-readable summary of the error, like 'Not Found'.       // 50
 * @param {String} [details] Optional.  Additional information about the error, like a textual stack trace. // 51
 */                                                                                                         // 52
...
if (document.addEventListener) {                                                                            // 11
  var event = Meteor.isCordova ? 'deviceready' : 'DOMContentLoaded';                                        // 12
  document.addEventListener(event, ready, false);                                                           // 13
  window.addEventListener('load', ready, false);                                                            // 14
} else {                                                                                                    // 15
  document.attachEvent('onreadystatechange', function () {                                                  // 16
    if (document.readyState === "complete")                                                                 // 17
      ready();                                                                                              // 18
  });                                                                                                       // 19
  window.attachEvent('load', ready);                                                                        // 20
}                                                                                                           // 21
                                                                                                            // 22
/**                                                                                                         // 23
 * @summary Run code when a client or a server starts.                                                      // 24
 * @locus Anywhere                                                                                          // 25
 * @param {Function} func A function to run on startup.                                                     // 26
 */                                                                                                         // 27
Meteor.startup = function (cb) {                                                                            // 28
  var doScroll = !document.addEventListener &&                                                              // 29
    document.documentElement.doScroll;                                                                      // 30
                                                                                                            // 31
  if (!doScroll || window !== top) {                                                                        // 32
    if (loaded)                                                                                             // 33
      cb();                                                                                                 // 34
    else                                                                                                    // 35
      queue.push(cb);                                                                                       // 36
  } else {                                                                                                  // 37
    try { doScroll('left'); }                                                                               // 38
    catch (e) {                                                                                             // 39
      setTimeout(function() { Meteor.startup(cb); }, 50);                                                   // 40
      return;                                                                                               // 41
    };                                                                                                      // 42
    cb();                                                                                                   // 43
  }                                                                                                         // 44
